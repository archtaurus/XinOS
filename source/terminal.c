...
uint8_t terminal_padding;
color_t terminal_foreground_color;
color_t terminal_background_color;
color_t terminal_color;

/* 返回指定位置上字符的序号 */
static inline size_t entry_index(size_t row, size_t col) {
    return (80 * row + col) * 2;
}

static inline color_t entry_color(color_t fgcolor, color_t bgcolor){
    return  bgcolor << 4 | fgcolor;
}

static inline uint16_t entry(uint8_t ch, color_t color) {
...
/* 清空屏幕 */
void cls(void) {
    uint16_t  e = entry(terminal_padding, terminal_color);
...
void putcat(char ch, size_t row, size_t col, uint8_t color) {
    size_t i = entry_index(row, col);
    if(color) {
        ((uint16_t *)terminal_buffer_top)[i] = entry(ch, color);
    } else {
...
void putsat(const char *str, size_t row, size_t col, uint8_t color) {
    volatile uint8_t *p = (volatile uint8_t *)terminal_buffer_top + entry_index(row, col);
    while(*str != 0) {
        *p++ = *str++;
        if(color) *p = color;
        p++;
